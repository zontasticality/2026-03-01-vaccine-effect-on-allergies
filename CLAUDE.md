...
- **`sources.md`** — Master list of all sources found. For each:
  - Title, author(s), year
  - How it was found (search query, citation chain, etc.)
  - Status: `[downloaded]`, `[read-full]`, `[read-partial: sections X,Y]`,
    `[scouted-only]`, `[not-yet-read]`
...
4. Scout each source (abstract/TOC), update `sources.md`.
